...
for text in df['clean_text']:
    examples = create_missing_word_examples(text, tokenizer_vocab_df, 25, 25, max_sequence_length)
    if examples == 0:
        continue
...
essay_representation_file = os.path.join(output_folder, 'essays_tensor_representation_max' + str(max_sequence_length) + '_25miss.npy')
...
position_index_pairs_file = os.path.join(output_folder, 'position_index_pairs_max' + str(max_sequence_length) + '_25miss.npy')
...
df.to_csv(os.path.join(output_folder, 'essays_with_positions_max' + str(max_sequence_length) + '_25miss.csv'), index=False)
